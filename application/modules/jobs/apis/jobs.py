...
def _auth_dep():
    return [] if settings.DISABLE_AUTH else [DependPermisson]


@router.post("/add/", summary="投递加法任务", dependencies=_auth_dep())
...
@router.post("/long-running/", summary="投递长任务", dependencies=_auth_dep())
async def enqueue_long_running(body: LongRunningSerializers):
    task_id = service.enqueue_long_running(body.seconds)
    return KeelResponse(data={"task_id": task_id}, msg="任务已投递")


@router.get("/{task_id}/", summary="查询任务状态", dependencies=_auth_dep())
